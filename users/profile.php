...
$query=mysqli_query($con,"update users set fullName='$fname',contactNo='$contactno',address='$address',campus='$campus',country='$country',pincode='$pincode' where idNumber='".$_SESSION['login']."'");
if($query)
{
...
<html lang="en">
  <head>
    ...
    <title>YNJGY | User Change Password</title>
...
<label class="col-sm-2 col-sm-2 control-label">User idNumber </label>
 <div class="col-sm-4">
<input type="idNumber" name="idNumber" required="required" value="<?php echo htmlentities($row['idNumber']);?>" class="form-control" readonly>
</div>
 </div>
...
<div class="col-sm-4">
<select name="campus" required="required" class="form-control">


<option value="<?php echo htmlentities($row['campus']);?>"><?php echo htmlentities($st=$row['campus']);?></option>
<?php $sql=mysqli_query($con,"select campusName from campus ");
while ($rw=mysqli_fetch_array($sql)) {
  ...
  {
  ?>
  <option value="<?php echo htmlentities($rw['campusName']);?>"><?php echo htmlentities($rw['campusName']);?></option>
<?php
}}
?>
